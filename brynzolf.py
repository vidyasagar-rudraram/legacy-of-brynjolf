...
class Brynzolf:
    """Brynzolf is a thief"""
    def __init__(self, room, brynzolf, guards, exit):
        self.SIZE = len(room)
        self.room = room
        self.brynzolf = (self.x, self.y) = brynzolf
        self.guards = guards
        self.exit = exit
    ...
    def move_brynzolf_back(self, last_position):
        path = path_dict[last_position]
        px, py = path[0], path[1]
    ...
    def solve_room(self, sx, sy):
        '''
            if destination E is reached, Brynzolf got out
        ...
        if self.is_valid(sx, sy):
            if (self.room[sx][sy] != "X" or self.room[sx][sy] != "G"):
                ...
                if self.sol_path:
                    ...
                    self.move_brynzolf_back(self.sol_path[-1])
    ...
    def walkroom(self, path):
        ...
        if self.is_valid(wx, wy):
            ...
            if inp_sol_path:
                ...
                self.move_brynzolf_back(inp_sol_path[-1])
